...
def main() -> None:
    """
    Basic system health script for the private server node.

    Writes a JSON line per run to health.log and prints the latest report.
    """
    ...
    disk = shutil.disk_usage(
        settings.base_dir if settings.base_dir.exists() else Path("/")
    )
